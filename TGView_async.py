...
f2.subplots_adjust(left=0.12, right=0.95, bottom =0.2, top=0.85)
f1.subplots_adjust(left=0.12, right=0.95, bottom =0.2, top=0.85)
...
class RPiReader(tk.Tk): 
    def __init__(self, *args, **kwargs): #args are variables. kwargs are keyboard args (dictionarys and such)
        ...
        self.myIcon = ImageTk.PhotoImage(file='/home/pi/Desktop/JoelPi/qam_logo_icon.png')
        ...
        p = os.popen("ps aux | grep '/home/pi/Desktop/JoelPi/TGView.py' | cut -c 43-44")
        p = p.read()
        #print(p)
        if "4" in p or "3" in p:
            fuckitup('start')
        ...
        async def serial_checker():
            global deltafConnected
            global meecoConnected
            global comPortOxygen
            testComOxygen =await get_O2(comPortOxygen = '/dev/ttyUSB1')
            ...
            try:
                if 'e' in testComOxygen:
                    ...
                    try:
                        testComOxygen =await get_O2(comPortOxygen)
                        deltafConnected = True
            ...
            try:
                testComMoisture = raw_to_ppb(await get_h20(comPortMoist))
                comPortMoist = '/dev/ttyUSB0'
                meecoConnected = True
            except:
                try:
                ...
                print(meecoConnected)
            await asyncio.sleep(1)
            #time.sleep(1)
        loop5 = asyncio.new_event_loop()
        asyncio.set_event_loop(loop5)
        try:
            print('starting serial check')
            s = serial_checker()
            print('entering event loop')
            loop5.run_until_complete(s)
...
def fuckitup(how):
    ...
    msg = Message(fuck, text = "Error has been logged/nPlease restart TGView", width = 300, bg="grey25",fg="grey85",font=SMALL_FONT,)
    msg.pack()
    button = Button(fuck,text="Click to Exit",command=fuckit, width = 20, height = 5)
    button['font'] = LARGEST_FONT
    button.pack()
...
### Start Recording function: writes o2, h20, and time variables to csv files      #moved from PageOne to confirm_fields  
    ...
    def stop(self):
        ...
        global path
        directory = "/home/pi/TGView"
        path = directory + '/' + str(header_list[3])+ "_" + str(header_list[4]) + "_" + str(header_list[2]) + "_" + str(header_list[0]) + "_" + start_timee
        global pathF
...
def animateh2o(i):
        ...
        if cycleH2O == 15:
            ...
            if var2.get() == 'radBoth':
                    ...
                    a2.plot(h2oxList, h2oyList,color='#2FA4FF',marker='o', picker=5)
...
def animateo2(i):   #### animation function.  
        ...
        async def o2dataGrab():
            ...
            if cycleO2 == 15:
                ...
                if var2.get() == 'radH2O':
                        ...
                        a1.set_title("No Data Recieved", fontsize=32, pad=20)
                        a1.set_xlabel('Time (minutes)')
                        a1.set_ylabel("N/A")
                o2fileTitle = "O2"
...
def exportH2O(startscreen):
    ...
    pdf.image('/home/pi/Desktop/JoelPi/QAM-Letter12.gif', x=10, y=10, w=100, h=24) #------- ADJUST THE FILEPATH BASED ON WHERE THE IMAGE IS LOCATED
...
def manage_pdf():
        ...
        def openCSV():
            ...
            # ------------------------------------------------------------------------------ #
                ...
                oldStopTimeRO2 = oldStartTimeRO2 + timedelta(hours = int(oldStopTimeHours), minutes = int(oldStopTimeMins), seconds = int(oldStopTimeSec))
...
def h2oloop(loop1):
    #asyncio.set_event_loop(loop1)
    loop1.run_until_complete(h2odataGrab())

app = RPiReader()
try:
    print('entering h2oloop')
    
    #loop1 = asyncio.get_event_loop()
    t = threading.Thread(target=h2oloop,args=(loop1,))
    t.start()
    
    
    #cors = asyncio.wait(h2odataGrab())
    #loop1.run_until_complete(h2odataGrab())
    #task = loop1.create_task(h2odataGrab())
    #loop1.run_until_complete(h2oloop())
    print('exiting h2o loop')
except Exception as e:
    print(e)
finally:
    print('wtf')
